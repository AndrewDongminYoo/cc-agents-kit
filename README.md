...
Unquoted, `$1`, `$@` or `$*` splits each argument into words as the default `IFS` would; in a command that mentions `IFS` at all the split is unknown, so the parameter is left unresolved and a literal argument holding `commit` is refused.
...
When the program name itself is a variable or a command substitution (`$g commit`, `$(command -v git) commit`) there is no candidate to scan, so the command is refused if `commit` stands where git reads its subcommand, counting the word after one of git's own options that take a separate value as that value (`$g --git-dir .git commit`).
...
- **`staged-secret-guard` only sees a commit the command spells out.** A function or alias from your shell profile, an alias in a git config file (`git ci`), a script, `sh -c` or `eval` strings, a runner such as `exec`, `nohup`, `sudo` or `xargs`, anything inside backticks, a command substitution inside double quotes (`out="$(git commit …)"`), a redirection before the subcommand (`git >/dev/null commit`), and an `IFS` assigned under a name the command never spells out (`printf -v "${v}FS" :`) all reach `git commit` without being recognised. A git `pre-commit` hook sees every one of these, because they all end in the same place.
- **`staged-secret-guard` reads a heredoc's text as commands.** A script or Makefile written through a heredoc is parsed as though it ran, so a line in it that commits is scanned, and one that names its program through a variable (`$GIT commit`, `$(GIT) commit`) is refused outright. Write such files with a tool other than `Bash`.
