...
- **`staged-secret-guard` only sees a commit the command spells out.** A function or alias from your shell profile, an alias in a git config file (`git ci`), a script, `sh -c` or `eval` strings, `xargs`, a program name built with backticks, and a command substitution inside double quotes (`out="$(git commit …)"`) all reach `git commit` without being recognised. A git `pre-commit` hook sees every one of these, because they all end in the same place.
