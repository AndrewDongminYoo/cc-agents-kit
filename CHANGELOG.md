...
- `guard-hooks` 0.3.1 `staged-secret-guard` follows a shell function defined in the same command.
  ...
  An unquoted `$1`, `$@` or `$*` splits each word as bash's default `IFS` does, so `run() { echo "+ $*"; $*; }; run git commit -m x` and `run "git commit -m x"` through `$@` are scanned too; in a command that mentions `IFS` the split is unknown, so the parameter is left unresolved and a literal word holding `commit` is refused (`f() { IFS=:; git $1; }; f commit:-m:x`).
