...
- `staged-secret-guard` recognises a git command that follows a shell keyword. `if git commit ...`, and any invocation in the body of a `for`/`while` loop, were treated as though the keyword were the command and went unscanned. Present before this release; found by replaying every git command in a local transcript archive against both versions.
